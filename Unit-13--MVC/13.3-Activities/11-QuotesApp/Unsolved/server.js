...
app.get("/:id", function(req, res) {
  connection.query("SELECT * FROM quotes WHERE id=?;", [req.body.quote],function(err,data){
    if (err) {
      return res.status(500).end();
    }
...
app.post("/api/quotes", function(req, res) {
  connection.query("INSERT INTO quotes (author) VALUES (?)", [req.body.author], function(err, result) {
    if (err) {
      return res.status(500).end();
    }
});
